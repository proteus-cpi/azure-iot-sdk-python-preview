...
class IotHubMQTTConverter(PipelineStage):
    ...
    def _handle_pipeline_event(self, event):
        ...
        if isinstance(event, pipeline_events_mqtt.IncomingMessage):
            ...
            if mqtt_topic.is_c2d_topic(topic, self.device_id):
            ...
            elif mqtt_topic.is_input_topic(topic, self.device_id, self.module_id):
                message = Message(event.payload)
                mqtt_topic.extract_properties_from_topic(topic, message)
                ...
                logger.info("Warning: unknown topic {} passing up to next handler".format(topic))
