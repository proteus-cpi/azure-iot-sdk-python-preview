...
class GenericIoTHubClient(AbstractIoTHubClient):
    ...
    def __init__(self, pipeline_adapter):
        ...
        :param pipeline_adapter: The pipeline_adapter that the client will use.
        """
        super().__init__(pipeline_adapter)
        self._inbox_manager = InboxManager(inbox_type=AsyncClientInbox)
        self._pipeline_adapter.on_transport_connected = self._on_state_change
        self._pipeline_adapter.on_transport_disconnected = self._on_state_change
        self._pipeline_adapter.on_transport_method_request_received = (
            self._inbox_manager.route_method_request
        )

    def _on_state_change(self, new_state):
        """Handler to be called by the pipeline_adapter upon a connection state change."""
    ...
    def _on_disconnected(self):
        """Helper handler that is called upon a a pipeline_adapter disconnect"""
    ...
    async def connect(self):
        ...
        connect_async = async_adapter.emulate_async(self._pipeline_adapter.connect)
    ...
    async def disconnect(self):
        ...
        disconnect_async = async_adapter.emulate_async(self._pipeline_adapter.disconnect)
    ...
    async def send_event(self, message):
        ...
        send_event_async = async_adapter.emulate_async(self._pipeline_adapter.send_event)
    ...
    async def receive_method_request(self, method_name=None):
        ...
        """
        if not self._pipeline_adapter.feature_enabled[constant.METHODS]:
    ...
    async def send_method_response(self, method_response):
        ...
        send_method_response_async = async_adapter.emulate_async(
            self._pipeline_adapter.send_method_response
        )
    ...
    async def _enable_feature(self, feature_name):
        ...
        enable_feature_async = async_adapter.emulate_async(self._pipeline_adapter.enable_feature)
...
class IoTHubDeviceClient(GenericIoTHubClient, AbstractIoTHubDeviceClient):
    ...
    def __init__(self, pipeline_adapter):
        super().__init__(pipeline_adapter)
        self._pipeline_adapter.on_transport_c2d_message_received = (
    ...
    async def receive_c2d_message(self):
        ...
        """
        if not self._pipeline_adapter.feature_enabled[constant.C2D_MSG]:
...
class IoTHubModuleClient(GenericIoTHubClient, AbstractIoTHubModuleClient):
    ...
    def __init__(self, pipeline_adapter):
        super().__init__(pipeline_adapter)
        self._pipeline_adapter.on_transport_input_message_received = (
    ...
    async def send_to_output(self, message, output_name):
        ...
        send_output_event_async = async_adapter.emulate_async(
            self._pipeline_adapter.send_output_event
        )
    ...
    async def receive_input_message(self, input_name):
        ...
        """
        if not self._pipeline_adapter.feature_enabled[constant.INPUT_MSG]:
