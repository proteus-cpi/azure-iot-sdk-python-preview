...
class StateBasedMQTTProvider:
    ...
    def _execute_action(self, action):
        ...
        if isinstance(action, SendMessageAction):
            ...
            logger.info("running PublishAction topic=%s qos=%s", action.topic, action.qos)
            self._mqtt_provider.publish(
                topic=action.topic, payload=action.message, qos=action.qos, callback=action.callback
            )
    ...
    def _create_mqtt_provider(self):
        ...
        self._mqtt_provider = MQTTProvider(
            client_id=client_id, hostname=hostname, username=username, ca_cert=None
        )
    ...
    def publish(self, topic, message, callback=None):
        ...
        """
        action = PublishAction(publish_topic=topic, message=message, callback=callback)
    ...
    def send_output_event(self, topic, message, callback=None):
        """
        Send an output message to the service.

        :param callback: callback which is called when the message publish has been acknowledged by the service.
        """
        action = SendMessageAction(topic=topic, message=message, callback=callback)
        self._trig_add_action_to_pending_queue(action)
