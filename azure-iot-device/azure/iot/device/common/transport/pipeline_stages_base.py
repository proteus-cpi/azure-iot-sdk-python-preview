...
class CoordinateRequestAndResponse(PipelineStage):
    """
    Pipeline stage which is responsible for coordinating IotRequestAndResponse operations.  For each
    IotRequestAndResponse operation, this stage passes down a SendIotRequest operation and waits for
    ...
    def _run_op(self, op):
        if isinstance(op, pipeline_ops_base.IotRequestAndResponse):
            # Convert IotRequestAndResponse operation into a SendIotRequest operation
    ...
    def _handle_pipeline_event(self, event):
        if isinstance(event, pipeline_events_base.IotResponseEvent):
            # match IotResponseEvent events to the saved dictionary of IotRequestAndResponse
            ...
            if event.request_id in self.pending_responses:
                ...
                # OPEN ISSUE: what to do if we don't have a matching $rid?  Drop it, error, i
                # or pass it up?
                pass
        else:
